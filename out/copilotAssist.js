...
4. Best practices to follow`;
    try {
        ...
        try {
            ...
            editor.selection = new vscode.Selection(new vscode.Position(0, 0), new vscode.Position(editor.document.lineCount, 0));
            await vscode.commands.executeCommand('inlineChat.start');
        ...
        vscode.window.showInformationMessage(`Could not automatically send task "${taskName}" to Copilot. Please manually open Copilot Chat and paste the following prompt:`, { modal: true, detail: finalPrompt });
        return false;
    }
    catch (err) {
        outputChannel.appendLine(`All methods failed when sending task: ${err instanceof Error ? err.message : String(err)}`);
        vscode.window.showErrorMessage(`Failed to send task to Copilot: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
}
